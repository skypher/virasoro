...
Verifies, in exact arithmetic:
  ...
  * the fusion normalization (6.1) at a generic sample point b = 2 for
    the labels (2,1) and (1,2), by solving for the normalized level-two
    singular vector from scratch;
  * the crossing normalization (6.1a) at b_0 for the labels
...
def fusion_normalization_generic():
    """Equation (6.1) at the generic sample point b = 2."""
    bval = sp.Integer(2)
    ...
    for (r, s) in [(2, 1), (1, 2)]:
        ...
        check_equal(f"(6.1) at generic b=2 for {(r, s)}",
                    element, fusion_polynomial(r, s, bval))


def crossing_normalization_ising():
    """Equation (6.1a) at b_0 against the audited crossing scalars."""
    ...
    for label, scalar in audited.items():
        check_equal(f"(6.1a) at b_0 for {label}",
