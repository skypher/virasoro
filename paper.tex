...
collision occurs in the energy module at grade three, and its embedding
diamond produces the first double pole at grade seven.  Both its residue and
...
block by an exact algebraic confluence.  ``Pole-free'' here means free of the
...
\section{Local Smith exponents and transverse collisions}
...
\begin{remark}[Source and normalization]
...
written without choosing a square root of \(\Lambda^2\).  In that
normalization the singular vector has \(L_{-1}^{rs}\)-coefficient \(1\).
Consequently, if \(\chi_{r,s}(b)\) denotes that normalized vector, the Ward
recursion gives
\[
...
\]
Both sides are rational in \(b\), so the identity continues to every
...
\[
 ...
 \end{split}
\]
Every term on the right is finite and is evaluated at \(c=1/2\).  Thus this
is a complete finite recursive formula for the principal part; it is not
being claimed here that the finite-part polynomials \(Q_8\) and \(Q_6\) have
been expanded in closed form.
\end{theorem}
...
\begin{remark}[Exact replay and its scope]
...
The grade-ten script checks the algebraic confluence calculation; it does not
construct the \(42\times42\) Gram matrix, \(\chi_{10}\), or the second
crossing form independently.  The Smith profile at grade ten instead follows
from the determinant and embedding-diagram argument in
Proposition~\ref{prop:grade-ten-smith}.
\end{remark}
...
the coefficientwise confluence theorem.  An independent construction of the
grade-ten \(42\times42\) second crossing form would nevertheless be a useful
additional audit.
